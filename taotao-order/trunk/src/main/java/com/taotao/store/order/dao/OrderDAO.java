...
public class OrderDAO implements IOrder {
    ...
    @Override
    public PageResult<Order> queryOrderByUserNameAndStatus(String buyerNick, Integer status, Integer page,
            Integer count) {
        PageBounds bounds = new PageBounds();
        bounds.setContainsTotalCount(true);
        bounds.setLimit(count);
        ...
        bounds.setOrders(com.github.miemiedev.mybatis.paginator.domain.Order.formString("create_time.desc"));
        PageList<Order> list = this.orderMapper
                .queryListByWhere1(bounds, Where.build("buyer_nick", buyerNick) , Where.build("status", status) );
        return new PageResult<Order>(list.getPaginator().getTotalCount(), list);
    }

    @Override
    public PageResult<Order> queryOrderByUserNameAndTime(String buyerNick, Integer date, Integer page,
            Integer count) {
        PageBounds bounds = new PageBounds();
        bounds.setContainsTotalCount(true);
        bounds.setLimit(count);
        ...
        if (date==2) {
            PageList<Order> list = this.orderMapper
                    .queryListByWhere2(bounds, Where.build("buyer_nick", buyerNick) );
            return new PageResult<Order>(list.getPaginator().getTotalCount(), list);
        }if (date==2013) {
            PageList<Order> list = this.orderMapper
                    .queryListByWhere3(bounds, Where.build("buyer_nick", buyerNick) );
            return new PageResult<Order>(list.getPaginator().getTotalCount(), list);
        }if (date==2012) {
            PageList<Order> list = this.orderMapper
                    .queryListByWhere4(bounds, Where.build("buyer_nick", buyerNick) );
            return new PageResult<Order>(list.getPaginator().getTotalCount(), list);
        }if (date==3) {
            PageList<Order> list = this.orderMapper
                    .queryListByWhere5(bounds, Where.build("buyer_nick", buyerNick) );
            return new PageResult<Order>(list.getPaginator().getTotalCount(), list);
        }
        return null;
    }
