...
<body>
				...
				<div class="g-0">
					<div id="content" class="c-3-5">
						...
						<div class="mod-main mod-comm lefta-box" id="order02">
								...
								<div class="extra-r">
									<div class="search-01">
										<input id="ip_keyword" name="" type="text" class="s-itxt"
											...
											<a href="javascript:;" class="btn-13" onclick="OrderSearch()"
							...
							<div class="mc">
								<table class="tb-void">
									...
									<script type="text/javascript" language="javascript">
												...
												.change(
														function() {
															var sDate = $(
																	...
																	.val();
															window.location = '/ucenter/queryTime/'+ sDate+'.html';
														});
										$("#orderState")
												.change(
														function() {
															var oState = $(
																	...
																	.val();
															window.location = '/ucenter/queryStatus/'+ oState+'.html';
														});
										$("#submitDate").val(1);
										$("#orderState").val(4096);
										...
										$(function(){
											//页面加载后显示
											$("a.order-del").click(function(event){
												//弹出提示
												var isConfirm=window.confirm("您确定要删除该订单吗？");
												if (!isConfirm) {
													//不删除
										...
										<c:forEach items="${orders.data}" var="order">
											<tr class="tr-th">
												<td colspan="6"><span class="tcol1"> 订单编号: <a
														...
														href="http://www.taotao.com/ucenter/view/${order.orderId }.html"
														...
														title="联系客服"></a>
												</span> <c:if test="${order.status > 4 }">
														<div>
															<a href="/ucenter/my/orders/remove/${order.orderId}.html"
																clstag="click|keycount|orderinfo|order_del"
																onclick="myfunction()" class="order-del" title="删除">
															</a>
														</div>
													</c:if>
											</tr>
											<tr id="track2538292730" oty="0,1,70" class="tr-td">
												<td>
													<div class="img-list">
														<c:forEach items="${order.orderItems}" var="orderItem">
															<a
																href="http://www.taotao.com/item/${ orderItem.itemId}.html"
																class="img-box"
																clstag="click|keycount|orderinfo|order_product"
																target="_blank"> <img title="${ orderItem.title}"
																width="50" height="50" src="${ orderItem.picPath}"
																class="err-product"></a>
														</c:forEach>
													</div>
												</td>
												<td><div class="u-name">${order.orderShipping.receiverName }</div></td>
												<td>￥${order.payment }<br> <c:choose>
												...
												<td><span class="ftx-03"><fmt:formatDate
															value="${dateValue}" pattern="yyyy-MM-dd" /> <br> <fmt:formatDate
																value="${dateValue}" pattern="HH:mm:ss" /></span> <input
												...
												<td><span class="ftx-03"> <c:choose>
															...
															<c:otherwise>未知</c:otherwise>
														</c:choose><br /> <a target="_blank"
														href="http://www.taotao.com/ucenter/view/${order.orderId }.html"
														clstag="click|keycount|orderinfo|order_check">订单详情</a>

												</span></td>

												<td id="operate2538292730" class="order-doi" width="100">
													<c:if test="${order.status < 4 }">
														<a href="/ucenter/my/orders/cancel/${order.orderId}.html"
															clstag="click|keycount|orderinfo|order_del"
															onclick="ensureMoveOrderToRecycle(2538292730,'397FF574E089D5409E6CC8EF67129D65');">取消订单</a>
													</c:if></span><span id="doi2538292730"><br>
													<a
													...
													href="/ucenter/my/orders/buy/${order.orderId}.html"
													target="_blank">还要买</a>
							...
							<div class="mt10">
								<div class="pagin fr" id="pagin-btm"></div>
								<div class="clr"></div>
							</div>
						</div>
	...
	<form id="orderSearchForm" action="/ucenter/my/orders/search.html"
		method="post">
